...
Generate the JSON response according to the schema: { "strengths": [], "discussion_areas": [], "risk_factors": [], "recommendations": [] }`
      ...
      const cacheKey = match_id ? `mental_insights_${match_id}` : null;
    ...
    if (match_id) {
      ...
      if (matchRow) {
        ...
        const baseScore = existingAnalysis.chronicResult?.compatibility_score || 0.85;
        const mentalScoreScaled = compatibilityIndex / 100;
