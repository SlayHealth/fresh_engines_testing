...
export default function CoupleRadarComparison({ scoresA, scoresB }) {
  if (!scoresA && !scoresB) {
  ...
  if (!scoresB) {
    return (
      <div className="glass-panel" style={{ padding: '40px 20px', textAlign: 'center' }}>
        <h3 style={{ margin: '0 0 16px 0', color: 'var(--partner-a)' }}>Partner A Analyzed</h3>
        <p style={{ color: 'var(--muted)' }}>Waiting for Partner B's report to unlock Couple Radar Comparison...</p>
        <div className="animate-pulse" style={{ marginTop: '20px', width: '50px', height: '50px', border: '3px solid var(--partner-b)', borderRadius: '50%', margin: '20px auto' }}></div>
      </div>
    );
  }
  ...
  return (
    <div className="glass-panel" style={{ padding: '20px', height: '400px' }}>
      <h3 style={{ margin: '0 0 16px 0', fontSize: '1.2rem', textAlign: 'center' }}>Couple Radar Comparison</h3>
      <ResponsiveContainer width="100%" height="100%">
        <RadarChart cx="50%" cy="50%" outerRadius="70%" data={data}>
          ...
          <Legend />
          <Radar name="Partner A" dataKey="A" stroke="var(--partner-a)" fill="var(--partner-a)" fillOpacity={0.2} />
          <Radar name="Partner B" dataKey="B" stroke="var(--partner-b)" fill="var(--partner-b)" fillOpacity={0.2} />
        </RadarChart>
      </ResponsiveContainer>
    </div>
  );
